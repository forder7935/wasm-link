...
use wasm_link::{nem, Binding, Engine, Linker, PluginInstance};
...
fn links_each_type_erased_binding_cardinality() -> Result<(), wasmtime::Error> {
	...
	let bindings = fixtures::bindings();
    let binding: Binding<String, TestContext, AtMostOne<String, PluginInstance<TestContext>>> =
        Binding::new(
		bindings.root.package,
		HashMap::from([( bindings.root.name, bindings.root.spec )]),
		AtMostOne( None ),
	...
	let bindings = fixtures::bindings();
    let binding: Binding<String, TestContext, Any<String, PluginInstance<TestContext>>> =
        Binding::new(
		bindings.root.package,
		HashMap::from([( bindings.root.name, bindings.root.spec )]),
		Any( HashMap::new() ),
...
fn links_each_async_type_erased_binding_cardinality() -> Result<(), Box<dyn std::error::Error>> {
	futures::executor::block_on( async {
        ...
        let instance = fixtures::plugins_concurrent(&engine)
            .plugin
            .plugin
            .instantiate(&engine, &Linker::new(&engine), executor.clone())
            .await?;
