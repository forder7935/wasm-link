...
- Commands: `nix develop --quiet --command <cmd>` or `nix-shell -p {package} --run` for single-use packages. Use these for all commands except standard utilities (ls, cat, grep, rg, git, etc.).
