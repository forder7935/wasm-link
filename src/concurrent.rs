...
//!
//! Runtime operations mirror [`crate::sync`] but return futures and must be awaited.
//! Runtime-state types are distinct from [`crate::sync`] and cannot be mixed with
//! them in one plugin tree.
//!
...
//! # Core Concepts
//!
//! - [`crate::concurrent::Binding`]: An abstract contract declaring what an implementer exports and what a
//! 	consumer may import. Contains a package name, a set of interfaces, and plugged-in
//! 	plugin instances.
//!
//! - [`crate::concurrent::Interface`]: A single WIT interface with functions and resources. Note that
//! 	interfaces don't have a name field; their names are provided as keys of a `HashMap`
//! 	when constructing a [`crate::concurrent::Binding`]. This prevents duplicate interface names.
//!
//! - [`crate::concurrent::Plugin`]: A struct containing a wasm component and the runtime context made available
//! 	to host exports; their ids are provided as keys of a `HashMap` when constructing a
//! 	[`crate::concurrent::Binding`]. This prevents duplicate ids.
//!
...
//!
//! - **Plug**: A plugin's declaration that it implements a [`crate::concurrent::Binding`].
//!
//! - **Socket**: A plugin's declaration that it depends on a [`crate::concurrent::Binding`]. Cardinality is
...
//! 	)"# )?,
//! 	Context { resource_table: ResourceTable::new() },
//! ).link( &engine, linker, vec![ leaf_binding ], executor.clone() ).await?;
//!
...
//! let result = root_binding.dispatch( "example", "get-value", &[ /* args */ ] ).await?;
//! match result {
...
//! # use wasm_link::concurrent::{ Binding, Plugin };
...
//! 	.link( &engine, linker.clone(), vec![ binding_d.clone() ], executor.clone() ).await?;
//! let plugin_c = Plugin::new( Component::new( &engine, "(component)" )?, Context::new())
//! 	.link( &engine, linker.clone(), vec![ binding_d ], executor.clone() ).await?;
//!
...
//! 	.link( &engine, linker, vec![ binding_b, binding_c ], executor.clone() ).await?;
//! # let _ = plugin_a ;
//! # Ok(())
//! # })
//! # }
...
//! )"# )?, Context::new()).instantiate( &engine, &linker, executor.clone() ).await?;
//!
...
//! let Any( map ) = binding.dispatch( "root", "get-value", &[] ).await?;
//! assert_eq!( map.len(), 2 );
...
//! 	Set initially via [`crate::concurrent::Plugin::with_initial_fuel`] and per-call via
//! 	[`crate::concurrent::Plugin::with_fuel_limiter`].
//!
...
//! 	Set per-call via [`crate::concurrent::Plugin::with_epoch_limiter`].
//!
...
//! 	Set once at instantiation via [`crate::concurrent::Plugin::with_memory_limiter`].
//!
...
//! in the [`crate::Engine`] configuration. Memory limits require no engine configuration.
...
//! a trap.
//!
//! **Engine enabled but no limiter set.** If you enable fuel/epoch deadlines in the [`crate::Engine`]
//! but don't set a limiter on the [`crate::concurrent::Plugin`], the behavior mimics the wasmtime default.
//! - *Fuel*: Without [`crate::concurrent::Plugin::with_initial_fuel`], a fresh [`wasmtime::Store`]
...
pub type Binding<Id, Ctx, Plugins = crate::cardinality::ExactlyOne<Id, PluginInstance<Ctx>>> =
	crate::BindingBase<Id, Ctx, Plugins, PluginInstance<Ctx>>;

/// A concurrent binding with erased cardinality.
pub type BindingAny<Id, Ctx> =
	crate::BindingAnyBase<Id, Ctx, PluginInstance<Ctx>>;

/// A component and context configured for the concurrent runtime.
pub type Plugin<Ctx> = crate::PluginBase<Ctx, PluginInstance<Ctx>>;
